...
.topBanner .searchBar input[type="submit"] {
	...
	background: url('resources/img/searchIcon.svg') no-repeat center center;
...
.filter-button .active {
	background-color: #61ce4e !important;
	...
	<div class="row">
		<div class="col-md-12" style="padding: 0;">
			<article class="topBanner">
				<div class="topSkin"
					style="background: url('resources/img/bg_intro.jpg'); position: absolute; opacity: .6; background-size: cover; background-repeat: no-repeat; background-position: center center; z-index: 10;"></div>
				<h2 class="bannerTitle">
					"방구할때는 어딜 봐야 되는거야?"<br />ROOM 9 !!
				</h2>
				...
				<form action="#" method="get" class="searchForm">
					<div class="searchBar">
						<input type="text" class="search" name="search" value=""
							placeholder="" id="search" /> <input type="submit" class="submit"
							name="search" value="" /> <span class="placeholderText"
							id="placeholderText" style="display: block;">삼성동, 오피스,
							강남대로 등</span>
					</div>
				</form>
			</article>
	...
	<div class="row wow zoomIn" data-wow-duration="1s" data-wow-delay=".5s">
		<div align="center" style="padding: 2% 0;"
			...
			<button class="btn btn-default filter-button"
				data-filter="conference">#회의실</button>
		...
		<article class="placeListArea">
			<ul class="placeList wow fadeInUp" data-wow-duration="1s"
				...
				<c:forEach items="${list }" var="item" varStatus="loop">
					<li class="filter" id="category${item.r_no }"><a
						href="<c:url value='/reservation.room9?r_no=${item.r_no }&m_email=${sessionScope.emailid }'/>">
							<div class="imgArea">
								<img src="<c:url value='upload/${item.r_image_1 }'/>" />
							</div>
							<div class="tagArea">
								<span class="btn tag">#${item.r_tag_1 }</span> <span
									class="btn tag">#${item.r_tag_2 }</span> <span class="btn tag">#${item.r_tag_3
									}</span>
							</div>
							<div class="infoArea">
								...
								<p class="address text-muted"
									...
									<i class="fas fa-map-marker-alt"></i> ${item.r_address }
								</p>
							</div>
							<div class="subInfoArea">
								<span class="capacity" style="font-weight: bold;">수용인원 :
									<span style="color: #333333; font-weight: bold">${item.r_peoplecount}</span>
									명
								</span>
								<p class="priceAndTime">
									<span class="price">${item.r_money }원</span> <span class="time">/시간</span>
								</p>
							</div>
					</a></li>
				</c:forEach>
...
<c:forEach items="${list }" var="item" varStatus="loop">
	<script>
		var category = "${category[loop.index]}";

		for (var i = 0; i < category.split(",").length; i++) {
			switch (category.split(",")[i].trim()) {
			case "회의실":
				$("#category${item.r_no }").addClass("conference");
				break;
			case "파티룸":
				$("#category${item.r_no }").addClass("party");
				break;
			case "스터디룸":
				$("#category${item.r_no }").addClass("study");
				break;
			case "다목적홀":
				$("#category${item.r_no }").addClass("multi");
				break;
			case "공연장":
				$("#category${item.r_no }").addClass("show");
				break;
			case "연수원":
				$("#category${item.r_no }").addClass("training");
				break;
			case "작업실":
				$("#category${item.r_no }").addClass("work");
				break;
			case "연습실":
				$("#category${item.r_no }").addClass("practice");
				break;
			case "레저시설":
				$("#category${item.r_no }").addClass("leisure");
				break;
			case "카페":
				$("#category${item.r_no }").addClass("cafe");
				break;
			}
		}
	</script>
